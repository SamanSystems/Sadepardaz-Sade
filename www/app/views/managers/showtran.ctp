...
echo '
		...
		<div class="input text"><label for="Transaction">نام پرداخت کننده</label>'.$transaction['Onlinetran']['name'].'</div>
		<div class="input text"><label for="Transaction">ايميل پرداخت کننده</label>'.$transaction['Onlinetran']['email'].'</div>
		...
		echo '</div>
				<div class="input text"><label for="Transaction">توضيحات تراکنش</label>'.$transaction['Onlinetran']['desc'].'</div>';
	echo '</div>';
?>